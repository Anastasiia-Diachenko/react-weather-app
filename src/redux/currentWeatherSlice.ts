...
const currentWeatherSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder.addCase(fetchCurrentWeatherAction.pending, (state, _action) => {
      state["loading"] = true;
    });
    builder.addCase(fetchCurrentWeatherAction.fulfilled, (state, action) => {
      state["weather"] = action?.payload;
      state["loading"] = false;
      state["error"] = undefined;
    });
    builder.addCase(fetchCurrentWeatherAction.rejected, (state, action) => {
      state["loading"] = false;
